...
class generate_grid_interpolation:
    ...
    def __init__(self, 
        ...
        self.residue_name_list = self.solvent_name#+[self.solute_name]
        #print("residues",self.residue_name_list)
        ...
        try:
            cosolvent_name = [each_residue for each_residue in self.solvent_name if each_residue != self.solute_name][0]
        except IndexError:
        ...
        ## DEFINING RGB COLORS
        ## FIRST CHANNEL
        self.grid_rgb_data[..., 0] = self.num_dist[self.solute_name][:] # Water is red
        ...
        if self.map_type == '3channel_oxy' or self.map_type == '3channel_hydroxyl' or self.map_type == '4chan_hydroxyl':
            ...
            if self.map_type != 'solvent_only':
                ...
                self.grid_rgb_data[..., 1] = self.num_dist[cosolvent_name][:] # Solute is green#modified but wrong wait for correction
            else:
                ## ADDING COSOLVENT AS THE SECOND CHANNEL
                
                self.grid_rgb_data[..., 1] = self.num_dist[cosolvent_name][:] # Cosolvent is green
                #print("rgbdata",self.grid_rgb_data)
                #print("shapeofrgbdata",np.shape(self.grid_rgb_data))
        ## THIRD CHANNEL -- COSOLVENT IF WE INCLUDE REACTANT
        if self.map_type != 'solvent_only':
            self.grid_rgb_data[..., 2] = self.num_dist[cosolvent_name][:] # Cosolvent is blue
        print("griddd",np.shape(self.grid_rgb_data))
        ## FOURTH CHANNEL
    ...
    def calc_solute_displacements(self, traj):
        ...
        #print("shappe",np.shape(self.solute_positions),np.shape(self.COM_solute))
        self.solute_displacements =  self.solute_positions - self.COM_solute[:,0:1,:] # Updated 06/17/2019 -- correcting for COM measurements
...
#                                                                         atom_names = atom_names)[1][0] ## ASSUMING ONE RESIDUE
    ...
    def plot_rgb_data_per_index( self, frame = 0, index = 1):
        ...
        ## DEFINING RGB DATA
        grid_rgb_data = self.grid_rgb_data
...
if __name__ == "__main__":    
    ...
    # windows dir
    '''
    path2AnalysisDir=r"D:/RateNet/dataset/BIm-MeSCN-300"
    gro_file=r"200-10bimmescn-mdnvt-6ns-300-ml.gro" # Structural file <-- must be a pdb file!
    xtc_file=r"200-10bimmescn-mdnvt-6ns-300-ml.xtc" # r"mixed_solv_prod_first_32_ns_centered.xtc"       
    # "Liz's testset"
    '''
  
    
    '''
    path2AnalysisDir="/Volumes/Liz2T/SolventNetBackup/SolventNetData/2020_SolventNet_Chem_Sci/MD_Simulations/training_set/XYL/mdRun_403.15_6_nm_XYL_25_WtPercWater_spce_dioxane"
    gro_file="mixed_solv_prod.gro"
    xtc_file="mixed_solv_prod_first_20_ns_centered_with_10ns.xtc"
    '''
    path2AnalysisDir="/Volumes/exfat/RateNet/dataset/MIm-MeSCN-360"
    #print(path2AnalysisDir.replace('-','')[-3:].lower())
    gro_file=r"200-10"+path2AnalysisDir.replace('-','')[-11:-3].lower()+"-mdnvt-6ns-"+path2AnalysisDir.replace('-','')[-3:]+"-ml.gro" # Structural file <-- must be a pdb file!
    #xtc_file=r"200-10"+path2AnalysisDir.replace('-','')[-11:-3].lower()+"-mdnvt-6ns-"+path2AnalysisDir.replace('-','')[-3:]+"-ml.xtc" # r"mixed_solv_prod_first_32_ns_centered.xtc"
    xtc_file="cutted-10000.xtc"
    ### LOADING TRAJECTORY
    ...
    input_vars = {
            ...
            #'solvent_name':   ['DIO','XYL'] ,
            
             # Solute of interest tBuOH XYL
            'solute_name':             'MIM',
            'solvent_name':   ['MIM','MES'] , # Solvent of interest HOH 'HOH' , 'GVLL'
            'map_box_size'      : 4, # nm box length in all three dimensions
            'map_box_increment' : 0.2, # box cell increments
            'map_type'          : 'solvent_only', 
    ...
    #pickle_file_path="/Volumes/Liz2T/RateNet/pickle/200-10bimmescn-mdnvt-6ns-300-ml"
    #pickle_file_path=r"/Volumes/exfat/RateNet/pickle/200-10bimmescn-mdnvt-6ns-300-ml"
    pickle_file_path=r"/Volumes/exfat/RateNet/pickle/"+path2AnalysisDir[-13:]
