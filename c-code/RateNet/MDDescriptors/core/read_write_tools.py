...
### CLASS DEFINITIONS
    extract_gro: 
        extracts gro file information
    extract_itp: 
        inputs an itp file path and extracts all details about the itp file
    extract_top: 
        inputs topology file and extracts all information about topoloy
    read_xvg: 
        function that reads xvg files
    top_file:
        function that reads and writes topology files
    import_index_file:
        function that inputs gromacs index files
### GLOBAL VARIABLES:
...
class read_xvg:
    ...
    FUNCTIONS:
        read_xvg_lines: 
            reads the xvg file
        find_xvg_data: 
            finds the data within the xvg file
        print_summary: 
            prints the summary
        get_legend: 
    ...
    def __init__(self, xvg_file):
        ...
        ## GETTING LEGEND
        self.legend_list = self.get_legend()
    ...
    def get_legend(self,
                  legend_prefix = "@ s"):
        '''
        This function locates all legends
        INPUTS:
            self: [obj]
                class object
        OUTPUTS:
        ...
        '''
        ## LOOPING AND LOCATING LINES
        lines_with_legend = [ each_line for each_line in self.xvg_lines if each_line.startswith(legend_prefix)]
        
        ## GETTING LEGEND LIST
        legend_list = [ re.findall(r'"([^"]*)"', each_line)[0] for each_line in lines_with_legend]
    ...
    def get_df(self,):
        '''
        This function uses a legend to generate a dataframe.
        INPUTS:
            self: [obj]
                class object
        OUTPUTS:
        ...
        '''
        ## CREATING DATAFRAME
        xvg_file_df = pd.DataFrame(self.xvg_data,
                                   columns = ['time'] + self.legend_list)
        
        ## APPLYING NUMERIC
        xvg_file_df = xvg_file_df.apply(pd.to_numeric, errors='ignore')
        return xvg_file_df
...
######################################################
### CLASS FUNCTION TO EXTRACT TOPOLOGY INFORMATION ###
######################################################
class top_file:
    '''
    The purpose of this function is to load topology files and edit them. 
    INPUTS:
        path_top_file: [str]
            path to toplogy file
    OUTPUTS:
        
        
    FUNCTIONS:
        comment_out_top_files:
            searches through topology file and comments specific indices, given a 
            list of things to comment out.
        write_top:
            writes the topology output
        
    '''
    def __init__(self,
                 path_top_file,
                 ):
        self.path_top_file = path_top_file
        
        ## READING TOP FILE
        self.top_file_lines = read_file_as_line(self.path_top_file)
        return
        
    ## FUNCTION TO EDIT TOP FILE
    def comment_out_top_files(self, 
                              top_file_lines,
                              comment_out_list = []):
        '''
        The purpose of this function is to comment out top files
        INPUTS:
            top_file_lines: [list]
                top file lines
            comment_out_list: [list]
                list of commented out items desired
        OUTPUTS:
            updated_top_file_lines: [list]
                list of output with the specific indices commented out. 
                
            
        '''
        ## COPYING LINES
        updated_top_file_lines = top_file_lines[:]
        
        ## GETTING INDEXES WITH MATCHING
        indexes_match = [idx for idx, each_line in enumerate(top_file_lines) 
                             for each_itp in comment_out_list if each_itp in each_line ]
        
        ## LOOPING THROUGH EACH MATCH AND EDITING
        for each_index in indexes_match:
            updated_top_file_lines[each_index] = "; " +  updated_top_file_lines[each_index]
        
        return updated_top_file_lines
    
    ## FUNCTION TO PRINT TOP FILE
    def write_top(self,
                  output_path,
                  top_file_lines):
        '''
        The purpose of this function is to write the output topology file
        INPUTS:
            output_path: [str]
                path to the output topology
            top_file_lines: [list]
                list of topology lines
        OUTPUTS:
            a new text file with the topology file
        '''
        ## OPENING THE FILE
        with open(output_path, "w") as f:
            ## WRITING
            for each_line in top_file_lines:
                f.write("%s\n"%(each_line))
        
        return

#########################################
### CLASS FUNCTION TO READ INDEX FILE ###
#########################################
class import_index_file:
    '''
    The purpose of this function is to read the index file.
    INPUTS:
        path_to_index: [str]
    ...
    OUTPUTS:
        ...
        self.data_groups: [dict]
            dictionary with data groups
    FUNCTIONS:
        write: 
            function to write in an index file
    '''
    ## INITIALIZING
    def __init__(self, path_to_index):
        ...
        if os.path.exists(self.path_to_index) is True:
            ...
            ## PRINTING
            print("Note! %s does not exist!"%(path_to_index))
            print("Skipping loading of index file")
    ...
    def read(self):
        '''
        This function reads the index file and splits the lines into a list. 
        '''
        ## READING THE INDEX FILE
        with open(self.path_to_index,'r') as f:
            self.data = f.read().splitlines()
        return
    
    ## FUNCTION THAT SPLITS THE READING INTO GROUPS
    def split_to_groups(self):
        '''
        This function splits the data into groups.
        OUTPUTS:
            self.data_groups: [dict]
                dictionary with data groups
                    e.g.
                    ndx_file.data_groups.keys()
                    Out[567]: dict_keys(['System', 'DOPC', 'AUNP'])
        '''
        ## CREATING STORAGE FOR KEYS
        self.data_groups = {}
        ...
        if os.path.exists(self.path_to_index) is True:
            ...
            ## GETTING INDICES 
            indices_of_header = [ idx for idx, each_line in enumerate(self.data) 
                                        if each_line.startswith('[') ]
            
            ## GETTING INDICES OF DATA
            indices_of_data = []
            ## LOOPING THROUGH HEADER INDEX
            for idx, each_indices in enumerate(indices_of_header):
                ## 
                first_idx = indices_of_header[idx]+1
                if len(indices_of_header) != idx + 1:
                    last_idx = indices_of_header[idx+1]
                else:
                    last_idx = len(self.data)
                ## STORING INDICES OF DATA
                indices_of_data.append([first_idx,last_idx])
            
            ## STORING EACH HEADER
            for header_idx, each_header in enumerate(indices_of_header):
                ## GETTING HEADER IDX
                header_line = self.data[each_header]
                ## GETTING THE KEY
                header_line_split = header_line.split(' ')
                header_key = header_line_split[1]
                ## GETTING INDEX
                current_indices = indices_of_data[header_idx]
                ## GETTING THE DATA
                current_data = self.data[current_indices[0]:current_indices[1]]
                ## SPLITTING BY SPACE AND CREATING INT
                current_data_split = calc_tools.flatten_list_of_list(
                                            [each_line.split(' ') for each_line in current_data])
    
                ## CONVERTING TO NUMPY 
                current_data_split = np.array( list((filter(None, current_data_split))) ).astype(int)
                
                ## STORING TO GROUPS
                self.data_groups[header_key] = current_data_split
    ...
    ### FUNCTION TO WRITETO INDEX FILE
    def write(self, 
              index_key, 
              index_list,
              backup_file = True,
              ...
              n_chunks=15):
        '''
        This function is to write to the index file by adding a new group. 
        INPUTS:
            self: [obj]
                self object
            index_key: [str]
            ...
            index_list: [list]
                list of indices
            backup_file: [logical]
                True if you want to backup the files
            n_chunks: [int]
                chunks per index list
            verbose: [logical]
                True if you want to print out the details
        OUTPUTS:
            void. This will write into the index file
        '''
        
        ## CHECKING IF THE KEY IS PART OF THE DICT
        if index_key in self.data_groups:
            print("The index key is within the group!")
            print("Index key: %s"%(index_key))
            print("Skipping the addition of it")
        else:
            
            if backup_file is True:
                ## OPENING THE FILE
                path_dir = os.path.dirname(self.path_to_index)
                index_name = os.path.basename(self.path_to_index)
                name_without_ext = os.path.splitext(index_name)[0]
                
                ## GETTING BACKUP FILE
                backup_file_name = name_without_ext + "_backup.ndx"
                ## PATH TO BACKUP
                path_backup_file_name = os.path.join(path_dir,
                                                     backup_file_name)
                ## COPYING
                print("Backing up index file: %s"%(path_backup_file_name))
                shutil.copyfile(self.path_to_index, path_backup_file_name)
                
            ## SPLITTING THE INDEX LIST
            split_index_list = self.chunks(lst = index_list,
                                           n = n_chunks)
            
            ## WRITING TO FILE
            with open(self.path_to_index, 'a') as f:
                ## WRITE TO KEY
                f.write("[ %s ]\n"%(index_key))
                ## WRITING CHUNKS
                for each_index in split_index_list:
                    ## ADDING ONE TO INDEX B/C GRO FILE STARTS AT 1
                    each_index = np.array(each_index) + 1
                    
                    joined_array = ' '.join([str(value) for value in list(each_index)])
                    ## ADDING TO INDEX
                    f.write("%s \n"%(joined_array))
            
            ## PRINTING IT OUT
            if verbose is True:
                print("Adding %s keys with %d atoms"%(index_key, len(index_list)))
