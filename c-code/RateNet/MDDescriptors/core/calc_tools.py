...
In this script, we have functions that can operate across trajectories. General functions are listed below:
    ## TRAJECTORY TOOLS
        find_total_residues: 
            Finds the total number of residues and the corresponding indexes to them
        find_unique_residue_names: 
            Finds unique residue names of a trajectory
        find_atom_index: 
            Finds the atom index based on residue name and atom name
        find_atom_names: 
            Finds atom names given the residue name
        find_residue_atom_index: 
            Outputs residue and atom index for a residue of interest
        find_residue_heavy_atoms: 
            finds all heavy atoms of a given residue
        find_heavy_atoms_index_of_residues_list:
            finds all heavy atom indices for a given residue list
        find_all_heavy_atoms:
            finds all heavy atoms within a trajectory
        find_multiple_residue_index: 
            finds multiple residues given a list -- outputs index and total residues
        find_center_of_mass: 
            Calculates the center of mass of residue
        calc_ensemble_vol: 
            calculates ensemble volume given the trajectory
        create_atom_pairs_list: 
            creates atom pair list between two atom lists (very quick!)
        create_atom_pairs_with_self: 
            creates atom pair list for a single atom list (you are interested in atom-atom interactions with itself)
        find_water_index:
            finds all water index (atom indices)
        calc_pair_distances_with_self_single_frame: 
            calculates pair distances for a single frame
        calc_pair_distances_between_two_atom_index_list: 
            calculates pair distances given two list of atom indices
        compute_pair_distances_with_arbitrary_displacements:
            function that computes distances using any displacement vector and trajectory
        
    ## SPLITTING TRAJECTORY FUNCTIONS
        split_traj_function: 
            splits the trajectory and calculates a value. This works well if you are receiving memory errors
        split_traj_for_avg_std: 
            splits the trajectory so you can calculate an average and standard deviation
        calc_avg_std: 
            calculates average and std of a list of dictionaries
        calc_avg_std_of_list: 
            calculates average and standard deviation of a list
        split_list: 
            splits list evenly
        split_general_functions: 
            splits calculations based on generalized inputs *** useful for cutting trajectories and computing X.
        
    ## VECTOR ALGEBRA
        unit_vector: 
            converts vectors to unit vectors
        angle_between: 
            finds the angles between any two vectors in radians
        rescale_vector: 
            rescales vectors and arrays from 0 to 1
        find_nearest: 
            finds nearest value within an array
    ...
    ## MISCELLANEOUS
        ...
        find_theoretical_error_bounds:
            computes the theoretical error bounds
        get_converged_value_from_end:
            computes a converged value from end of y= line
...
### FUNCTION TO FIND ALL HEAVY ATOM INDEXES FOR A RESIDUE list
def find_heavy_atoms_index_of_residues_list(traj, residue_list = ['HOH']):
    '''
...
### FUNCTION TO FIND ONLY HEAVY ATOMS
def find_residue_heavy_atoms( traj, residue_name ):
    '''
    The purpose of this function is to find the residue heavy atoms. 
    INPUTS:
        traj: [obj]
            trajectory object
        residue_name: [str]
    ...
    OUTPUTS:
        heavy_atom_index: [np.array]
            atom index of all non-hydrogens
    '''
    ## GETTING ATOM INDEX
    res_index, atom_index = find_residue_atom_index( traj = traj,
                                                     residue_name = residue_name)
    ## FLATTIN ATOM INDEX
    atom_index_flatten = flatten_list_of_list(my_list = atom_index)
    ## FINDING ALL ATOMS THAT ARE NOT H
    heavy_atom_index = np.array([each_index for each_index in atom_index_flatten 
                                     if traj.topology.atom(each_index).element.symbol != 'H' ])
    return heavy_atom_index


### FUNCTION TO FIND ALL HEAVY ATOMS
def find_all_heavy_atoms(traj):
    '''
    The purpose of this function is to compute all heavy atoms.
    INPUTS:
        traj: [obj]
            trajectory object
    OUTPUTS:
    ...
    '''
    ## FINDING ALL ATOMS THAT ARE NOT H
    heavy_atom_index = np.array([each_atom.index for each_atom in traj.topology.atoms 
                                                 if each_atom.element.symbol != 'H' ])
    return heavy_atom_index
...
def find_center_of_mass( traj, residue_name = 'HOH', atom_names = None, combine_all_res = False, verbose = False):
    '''
    ...
    INPUTS:
        ...
        residue_name: [str]
            residue name as a string (i.e. 'HOH')
        atom_names: [list]
            list of atom names within your residue (i.e. ['O','H1','H2']). If None, then we will find all atom names for the residue.
        combine_all_res: [logical]
            True if you want the center of all residues together
    OUTPUTS:
    ...
    residue_index, atom_index = find_residue_atom_index(traj = traj, 
                                                        residue_name = residue_name, 
                                                        atom_names = atom_names)
    if atom_names is None:
        atom_names = [ traj.topology.atom(atom_ind).name for atom_ind in atom_index[0] ]
    if combine_all_res is True:
        atom_index = [flatten_list_of_list(atom_index)]
    ## FINDING MASS OF A SINGLE GROUP
    atom_mass = [ traj.topology.atom(atom_ind).element.mass for atom_ind in atom_index[0] ]
    ## FINDING TOTAL MASS
    ...
    if verbose is True:
        print_total_time(start_time = COM_time,
                         string = "Total time for COM calculation was",
                         )
...
## COMPUTING DISTANCES WITH MDTRAJ USING ARBITRARY ATOM INDICES
def compute_pair_distances_with_arbitrary_displacements(traj,
                                                        displacements,
                                                        periodic=True):
    '''
    The purpose of this function is to utilize MDTraj's distance method to 
    compute pair distances for an arbitrary set of displacement array. Note 
    that thi sfunction creates a copy of the trajectory, moves all the 
    atoms according to your displacement locations, then recomputes 
    the distances.
    INPUTS:
        traj: [md.traj] 
            trajectory from md.traj
        displacements: [np.array, shape = (num_frames, num_dist ,3)]
            displacement array in x, y, z
        periodic: [logical]
            True if you want periodic boundary conditions
    OUTPUTS:
        distances: [np.array, shape=(num_frames, num_pairs)]
            distance betwen atom pairs
        atom_pairs: [np.array, shape=(num_pairs, 1)]
            atom pairs based on the upper triangular matrix
        total_atoms: [int]
            total number of atoms or displacement vectors
        atom_indices_to_change: [np.array, shape=(num_atoms)]
            atom indices used for transformations
    '''
    
    ## START BY COPYING TRAJECTORY
    copy_traj = traj[:]
    
    ## GETTING TOTAL ATOMS
    total_atoms = displacements.shape[1]
    
    ## GETTING ARBITRARY ATOM INDICES
    atom_indices_to_change = np.arange(total_atoms)
    
    ## CHANGING THE DISPLACEMENT
    copy_traj.xyz[:,atom_indices_to_change,:] = displacements[:]
    
    ## DEFINING ATOM PAIRS
    atom_pairs, upper_triangular = create_atom_pairs_with_self(indices = atom_indices_to_change)
    
    ## GETTING DISTANCES
    distances = md.compute_distances(traj = copy_traj,
                                     atom_pairs = atom_pairs,
                                     periodic = periodic
                                     ) # shape=(num_frames, atom_pair))
    
    return distances, atom_pairs, total_atoms, atom_indices_to_change


### FUNCTION TO FIND THE DISTANCES FOR SINGLE FRAME
def calc_pair_distances_with_self_single_frame(traj, 
                                               atom_index, 
                                               frame=-1, 
                                               periodic = True):
    '''
    ...
    INPUTS:
        traj: [obj]
            trajectory from md.traj
        atom_index: [np.array, shape=(num_atoms, 1)] 
            atom indices that you want to develop a pair distance matrix for
        frame: [int, default=-1] 
            frame to calculate gold-gold distances
...
def calc_pair_distances_between_two_atom_index_list(traj, 
                                                    atom_1_index, 
                                                    atom_2_index, 
                                                    periodic=True ):
    '''
...
def find_nearest(array, value):
    '''
    The purpose of this function is to find the nearest value in an array
    INPUTS:
        array: [np.array]
            array to check to see if vlue is nearest
        value: [float]
            value to check for the closest value
    OUTPUTS:
        array[idx]: [float]
            value that has the closest in your array
        idx: [int]
            index within the array that found the nearest value
...
#### FUNCTION TO CHECK PBC
#def check_pbc_distances(distance_array,
#                        L):
#    '''
#    This function checks for periodic boundary conditions
#    '''
...
### FUNCTION TO GET BOUNDS OF THEORTEICAL ERROR
def find_theoretical_error_bounds(value, 
                                  percent_error = 5,
                                  convergence_type = "percent_error"):
    '''
    The purpose of this function is to get the theoretical value bounds. 
    This function uses the idea of:
        %error = | (calc - theo) / theo | * 100
    We can re-arrange and look for theoretical bounds
        calc = (theo * (%error/100 + 1))
    INPUTS:
        value: [float]
            value you are interested in
        percent_error: [float]
            percentage error, e.g. 5%
        convergence_type: [str]
            type of convergence
                percent_error: 
                    computes the percent error for the boudns
                value:
                    computes the value difference from actual - value in absolute value
            
    OUTPUTS:
        theoretical_bounds: [tuple, 2]
            theortical bounds 
        bound: [float]
            bound 
    '''
    ## DEFINING AVAILABLE TYPES
    available_types = ["percent_error", "value" ]
    ## COMPUTING PERCENT ERROR
    if convergence_type == "percent_error":
        expected_value = value * (percent_error/100 + 1)
        bound = abs(value - expected_value)
    elif convergence_type == "value":
        bound = percent_error
    else:
        print("Error! the convergence_type is not available. Below are the available types:")
        print("%s"%(", ".join(available_types)))
    
    ## GETTING THEORETICAL BOUNDS
    theoretical_bounds= (value-bound, value + bound)

    return theoretical_bounds, bound

### FUNCTION TO GET LAST X OR Y
def get_converged_value_from_end( y_array, 
                                  desired_y,
                                  bound,
                                  ):
    '''
    The purpose of this function is to get converged y value from the end. 
    This is useful for finding the earliest sampling time and so forth. 
    INPUTS:
        y_array: [np.array]
            array for y values
        desired_y: [float]
            y that is desired
        bound: [float]
            bound that could be deviated
    OUTPUTS:
        index: [int]
            index where the y_array ends (last index)
    '''
    ## COMPUTING DEVIATIONS
    deviations = y_array - desired_y
    ## GETTING INDICES OUTSIDE BOUND
    indices_outside_bound = np.where(np.abs(deviations)>bound)[0]
    if len(indices_outside_bound) > 0:
        ## GETTING LAST INDEX ADDING 1
        index = indices_outside_bound[-1] + 1
    else:
        index = 0
    return index

### FUNCTION TO COMPUTE DISPLACEMENTS FOR NVT ENSEMBLE
