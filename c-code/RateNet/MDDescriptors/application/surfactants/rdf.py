...
def compute_rdf( traj,
    ...
    if os.path.exists( path_pkl ) and rewrite is False:
        ...
        atom_pairs = np.array([ [ aa, ref_ndx ] for aa in atom_indices if aa != ref_ndx ])
    ...
    return np.stack(( r, g_r )).transpose()
...
class interfacial_rdf:
    ...
    def compute_single_frame( self, 
        ...
        atom_pairs = np.array([ [ ndx, self.ref_ndx ] for ndx in atom_indices ])
        r_dist = md.compute_distances( traj, atom_pairs = atom_pairs, periodic = self.periodic ).flatten()
